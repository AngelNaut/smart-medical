...
| QA-03 | Prueba de Prioridad Alta (Urgencia) | El algoritmo asigna automáticamente una puntuación matemática alta a un paciente simulado con condición grave. | No implementado aún | ⚠️ NO DESARROLLADO | Falta implementar algoritmo de priorización en PriorityScore. |
| QA-04 | Prueba de Prioridad Baja (Rutina) | El algoritmo asigna una puntuación matemática baja a un paciente simulado con chequeo de rutina o condición leve. | No implementado aún | ⚠️ NO DESARROLLADO | Falta implementar algoritmo de priorización en PriorityScore. |
| QA-05 | Verificación de la Cola (Ordenamiento) | Al inyectar 5 solicitudes con distintas gravedades, el backend las organiza en la base de datos poniendo las más críticas en primer lugar. | No implementado aún | ⚠️ NO DESARROLLADO | Falta implementar lógica de ordenamiento por prioridad. |
| QA-06 | Factor del Tiempo de Espera | Al modificar la fecha de creación de una cita a una más antigua, el sistema aumenta su prioridad por tiempo de espera. | No implementado aún | ⚠️ NO DESARROLLADO | Falta implementar lógica de incremento por tiempo. |
